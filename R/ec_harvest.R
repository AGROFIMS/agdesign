get_ec_harv <- function(allinputs, addId){
  ...
  for( i in 1:length(addId)){
    harv_temp  <- rbind(harv_temp, harv %>% filter(str_detect(id, addId[i])))
  }
  harv <- harv_temp
...
get_ec_harv_inter <- function(allinputs, addId, crop){
  
  #crop<-crop #TODO: Utilizar como nombre de las listas
  ...
  for(i in 1:length(addId)){
    chr<- as.character(i)
    lgl_header_harv <- str_detect(string = names(dt_harv), chr )
                                                        # replacement = addId[i] )  
    dt_harv_inter[[i]] <- dt_harv[,lgl_header_harv] 
    names(dt_harv_inter[[i]]) <- str_replace_all(names(dt_harv_inter[[i]]),pattern = chr,
                                                        replacement = "1"  )
  }
  
  names(dt_harv_inter) <- crop
  dt_harv_inter
